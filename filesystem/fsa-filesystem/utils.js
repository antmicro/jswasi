...
export async function initMetadataPath(handle) {
    const components = await (await navigator.storage.getDirectory()
    // @ts-ignore
    ).resolve(handle);
    return components.join("/");
}
export async function getTopLevelHandle(name, create) {
...
export function mapErr(e, isDir) {
    switch (e.name) {
