...
export class TopLevelFs {
    ...
    async getDescInfo(path, workerId, dirflags = 0, oflags = 0, fs_rights_base = constants.WASI_RIGHTS_ALL, fs_rights_inheriting = constants.WASI_RIGHTS_ALL, fdflags = 0, symlink_depth = SYMBOLIC_LINK_DEPTH_LIMIT) {
        let rpath = realpath(path);
        let lastSeparator, fs;
        for (lastSeparator = rpath.length; lastSeparator > 0; lastSeparator = rpath.lastIndexOf("/", lastSeparator - 1)) {
        ...
        if (fs === undefined)
            fs = this.mounts["/"];
        if (fs === undefined)
            throw new Error("No filesystem mounted at root");
        let { err, index, desc } = await fs.open(rpath.slice(lastSeparator), dirflags, oflags, fs_rights_base, fs_rights_inheriting, fdflags, workerId);
        if (desc) {
            ...
            switch (err) {
                ...
                case constants.WASI_ENOTDIR: {
                    ...
                    if (fdstat !== undefined &&
                        ...
                        return await this.getDescInfo(__path, workerId, dirflags, oflags, fs_rights_base, fs_rights_inheriting, fdflags, symlink_depth - 1);
                    }
                    break;
                }
                default: {
    ...
    async open(path, dirflags = 0, oflags = 0, fdflags = 0, fs_rights_base = constants.WASI_RIGHTS_ALL, fs_rights_inheriting = constants.WASI_RIGHTS_ALL, workerId = -1) {
        return await this.getDescInfo(path, workerId, dirflags, oflags, fs_rights_base, fs_rights_inheriting, fdflags);
    }
    async openat(desc, path, dirflags = 0, oflags = 0, fdflags = 0, fs_rights_base = constants.WASI_RIGHTS_ALL, fs_rights_inheriting = constants.WASI_RIGHTS_ALL, workerId = -1) {
        let __path = this.abspath(desc, path);
        return await this.getDescInfo(__path, workerId, dirflags, oflags, fs_rights_base, fs_rights_inheriting, fdflags);
    }
    async createDir(path, desc = undefined, workerId = -1) {
        let __path = this.abspath(desc, path);
        if (__path.endsWith("/"))
            __path = __path.slice(0, -1);
        const { desc: __desc, fs, err, } = await this.getDescInfo(dirname(__path), workerId, constants.WASI_LOOKUPFLAGS_SYMLINK_FOLLOW);
        if (err !== constants.WASI_ESUCCESS)
            return err;
    ...
    async addSymlink(target, linkpath, linkdesc = undefined, workerId = -1) {
        let path;
        ...
        const { desc, fs, err } = await this.getDescInfo(dirname(path), workerId, constants.WASI_LOOKUPFLAGS_SYMLINK_FOLLOW);
        if (err !== constants.WASI_ESUCCESS)
            return err;
        return fs.symlinkat(target, desc, basename(linkpath));
    }
    async removeEntry(path, is_dir, desc = undefined, workerId = -1) {
        let __path = this.abspath(desc, path);
        const { desc: __desc, fs, err, } = await this.getDescInfo(dirname(__path), workerId, constants.WASI_LOOKUPFLAGS_SYMLINK_FOLLOW);
        if (err !== constants.WASI_ESUCCESS)
            return err;
    ...
    async move(desc_s, source, desc_t, target, workerId = -1) {
        ...
        if (Object.keys(this.mounts).some((key) => {
            key.startsWith(__source);
        }) ||
            __source.startsWith(__target))
            return constants.WASI_EBUSY;
        const dinfo1 = await this.getDescInfo(__source_dirname, workerId, constants.WASI_LOOKUPFLAGS_SYMLINK_FOLLOW, constants.WASI_O_DIRECTORY);
        // If parent directory descriptor of the source path cannot be found,
        // return with error
        if (dinfo1.err !== constants.WASI_ESUCCESS)
            return dinfo1.err;
        const __dinfo1 = await this.getDescInfo(__source, workerId, constants.WASI_LOOKUPFLAGS_SYMLINK_FOLLOW);
        // If the source path doesn't correspond to an existing filesystem entry,
        // return with error
        if (__dinfo1.err !== constants.WASI_ESUCCESS)
            return __dinfo1.err;
        const __res = await __dinfo1.desc.getFilestat();
        if (__res.err !== constants.WASI_ESUCCESS)
            return __res.err;
        const filestat1 = __res.filestat;
        const dinfo2 = await this.getDescInfo(__target_dirname, workerId, constants.WASI_LOOKUPFLAGS_SYMLINK_FOLLOW, constants.WASI_O_DIRECTORY);
        // If parent directory descriptor of the target path cannot be found,
        // return with error
        if (dinfo2.err !== constants.WASI_ESUCCESS)
            return dinfo2.err;
        const __dinfo2 = await this.getDescInfo(__target, workerId, constants.WASI_LOOKUPFLAGS_SYMLINK_FOLLOW);
        ...
        if (__dinfo2.err === constants.WASI_ESUCCESS) {
            ...
            if (filestat1.dev !== filestat2.dev)
        ...
        return dinfo2.fs.renameat(dinfo1.desc, basename(__source), dinfo2.desc, basename(__target));
    }
    async addMount(sourceDesc, sourcePath, targetDesc, targetPath, filesystemType, 
    // mountFlags is not used but it is present in linux and might
    // be useful in the future
    _mountFlags, data, workerId = -1) {
        const __targetPath = this.abspath(targetDesc, targetPath);
        let dinfoTarget;
        if (__targetPath !== "/") {
            dinfoTarget = await this.getDescInfo(__targetPath, workerId, constants.WASI_LOOKUPFLAGS_SYMLINK_FOLLOW, constants.WASI_O_DIRECTORY);
            if (dinfoTarget.err !== constants.WASI_ESUCCESS)
                return dinfoTarget.err;
        ...
        if (__sourcePath === "") {
            ...
            const dinfoSource = await this.getDescInfo(__sourcePath, workerId, constants.WASI_LOOKUPFLAGS_SYMLINK_FOLLOW, constants.WASI_O_DIRECTORY);
            if (dinfoSource.err !== constants.WASI_ESUCCESS)
                return dinfoSource.err;
            const mountResult = await dinfoSource.desc.mountFs(data);
            if (mountResult.err !== constants.WASI_ESUCCESS)
    ...
    async addMountFs(path, fs, workerId = -1) {
        ...
        let descInfo = await this.getDescInfo(path, workerId, constants.WASI_LOOKUPFLAGS_SYMLINK_FOLLOW, constants.WASI_O_DIRECTORY);
    ...
    removeMount(path) {
        ...
        if (Object.keys(this.mounts).every((mountPoint) => {
    ...
    async readLink(desc, path, workerId = -1) {
        let { err: __err, desc: __desc } = await this.openat(desc, path, 0, 0, 0, constants.WASI_RIGHTS_ALL, constants.WASI_RIGHTS_ALL, workerId);
