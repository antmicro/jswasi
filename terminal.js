...
async function essentialBins(tfs) {
    ...
    const checksumPromises = Object.entries(CHECKSUM_BINARIES).map(async ([filename, [address, checksum]]) => {
        ...
        if (file.err === constants.EXIT_SUCCESS) {
            ...
            let exp_sum = new TextDecoder().decode((await (await fetch(checksum)).arrayBuffer()).slice(0, 32));
...
async function getDefaultFdTable(tfs) {
    const descs = [
        await tfs.open("/dev/ttyH0", 0, 0, 0, constants.WASI_EXT_RIGHTS_STDIN, 0n),
        await tfs.open("/dev/ttyH0", 0, 0, constants.WASI_FDFLAG_APPEND, constants.WASI_EXT_RIGHTS_STDOUT, 0n),
        await tfs.open("/dev/ttyH0", 0, 0, constants.WASI_FDFLAG_APPEND, constants.WASI_EXT_RIGHTS_STDERR, 0n),
        await tfs.open("/"),
    ];
...
async function initFs(fs) {
    ...
    await Promise.all(Object.entries(ALWAYS_FETCH_FILES).map(([filename, address]) => fetchFile(fs, filename, address, true)));
}
...
const RECOVERY_MOUNT_CONFIG = {
    fsType: "vfs",
    opts: {},
};
async function getKernelConfig(tfs) {
...
function mountRootfs(tfs, mountConfig) {
    return tfs.addMount(undefined, "", undefined, "/", mountConfig.fsType, 0n, mountConfig.opts);
}
async function recoveryMotd(tfs) {
    const { err, desc } = await tfs.open("/etc/motd", constants.WASI_LOOKUPFLAGS_SYMLINK_FOLLOW, constants.WASI_O_CREAT | constants.WASI_O_TRUNC);
    if (err !== constants.WASI_ESUCCESS)
        return;
    await desc.write(new TextEncoder().encode("\n[WARNING] Could not mount filesystem, volatile filesystem used as root for recovery\n"));
}
...
export async function init(terminal) {
    if (!navigator.storage.getDirectory) {
        terminal.io.println("Your browser doesn't support File System Access API yet.");
    ...
    if ((await mountRootfs(tfs, kernelConfig.mountConfig)) !==
    ...
    const { err } = await tfs.open("/filesystem-initiated", constants.WASI_LOOKUPFLAGS_SYMLINK_FOLLOW, constants.WASI_O_CREAT | constants.WASI_O_EXCL);
    if (err === constants.WASI_ESUCCESS)
        await initFs(tfs);
    if ((await essentialBins(tfs)).some((err) => err !== constants.WASI_ESUCCESS)) {
        tfs.removeMount("/");
    ...
    await tfs.addMountFs("/dev", await createDeviceFilesystem(driverManager, processManager, {
        terminal,
        currentProcessId: 0,
    }));
    ...
    try {
        ...
        terminal.io.println(`Cannot create file descriptor table for init process: ${error}`);
        return;
    }
    await processManager.spawnProcess(null, // parent_id
    null, // parent_lock
    "/usr/bin/wash", fdTable, kernelConfig.init, DEFAULT_ENV, false, DEFAULT_WORK_DIR, { maj: 1 /* major.MAJ_HTERM */, min: 0 } // TODO: this should not be hardcoded
    );
}
export async function tearDown(println) {
