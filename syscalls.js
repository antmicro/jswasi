...
export default async function syscallCallback(event, processManager) {
    ...
    switch (action) {
        case "console": {
            console.log(`%c [dbg (%c${processName}:${processId}%c)] %c ${data}`, "background:black; color: white;", "background:black; color:yellow;", "background: black; color:white;", "background:default; color: default;");
            break;
        }
        case "proc_exit": {
            if (processManager.processInfos[processId].env["DEBUG"] === "1") {
                console.log(`%c [dbg (%c${processName}:${processId}%c)] %c exited with result code ${data}`, "background:black; color: white;", "background:black; color:yellow;", "background: black; color:white;", "background:default; color: default;");
            }
        ...
        case "getcwd": {
            const { bufLen, sharedBuffer } = data;
        ...
        case "spawn": {
            ...
            for (let i = 0; i < redirects.length && exit_staus == constants.EXIT_SUCCESS; i++) {
                ...
                switch (type) {
                    ...
                    case constants.WASI_EXT_REDIRECT_TYPE_READWRITE: {
                        ...
                        switch (type) {
                            ...
                            default: {
                                ...
                                console.log(`Spawn: program should not enter here for redirect type ${redirect.type}.`);
                                continue;
                            }
                        }
                        const { desc, err } = await processManager.filesystem.open(path, constants.WASI_LOOKUPFLAGS_SYMLINK_FOLLOW, openFlags, fdFlags, fdRights, fdRights, processId);
                    ...
                    case constants.WASI_EXT_REDIRECT_TYPE_DUPLICATE: {
                        if (fds.getDesc(redirect.fd_src) !== undefined) {
                            ...
                            console.log(`Spawn: cannot duplicate fd=${redirect.fd_src}, it is closed.`);
                            exit_staus = constants.EXIT_FAILURE;
                        }
                        break;
                    }
                    case constants.WASI_EXT_REDIRECT_TYPE_CLOSE: {
                        ...
                        if (desc !== undefined) {
                            ...
                            console.log(`Spawn: cannot close fd=${redirect.fd_src}, it is already closed.`);
                            exit_staus = constants.EXIT_FAILURE;
                        }
                        break;
                    }
            ...
            switch (path) {
                ...
                default: {
                    ...
                    let sigintOccurred = events !== null
                    ...
                    try {
                        const id = await processManager.spawnProcess(processId, background ? null : parentLck, path, fds, args, env, background, processManager.processInfos[processId].cwd);
                        Atomics.store(childPID, 0, id);
                        if (env["DEBUG"] === "1") {
                            const newProcessName = path.split("/").slice(-1)[0];
                            console.log(`%c [dbg (%c${newProcessName}:${id}%c)] %c spawned by ${processName}:${processId}`, "background:black; color: white;", "background:black; color:yellow;", "background: black; color:white;", "background:default; color: default;");
                        }
                    }
                    catch (_) {
        ...
        case "path_symlink": {
            const { sharedBuffer, targetPath, linkFd, linkPath } = data;
            const lck = new Int32Array(sharedBuffer, 0, 1);
            const { fds } = processManager.processInfos[processId];
            let err;
            if (fds.getDesc(linkFd) === undefined) {
                ...
                err = await processManager.filesystem.addSymlink(targetPath, linkPath, fds.getDesc(linkFd), processId);
            }
            Atomics.store(lck, 0, err);
            Atomics.notify(lck, 0);
            break;
        ...
        case "path_link": {
            ...
            if (fds.getDesc(oldFd) !== undefined) {
                err = (await fds
                    ...
                    .write(new TextEncoder().encode("hard links are not supported"))).err;
            }
            else {
                err = constants.WASI_EBADF;
            }
        ...
        case "path_readlink": {
            ...
            if (fds.getDesc(fd) !== undefined) {
                let __path;
                ({ err, path: __path } = await processManager.filesystem.readLink(fds.getDesc(fd), path, processId));
                if (err === constants.WASI_ESUCCESS) {
        ...
        case "fd_prestat_dir_name": {
            ...
            const { fds } = processManager.processInfos[processId];
            if (fds.getDesc(fd) !== undefined) {
                path.set(new TextEncoder().encode(basename(fds.getDesc(fd).getPath())), 0);
                err = constants.WASI_ESUCCESS;
            }
            else {
                err = constants.WASI_EBADF;
        ...
        case "fd_write": {
            ...
            if (fds.getDesc(fd) !== undefined) {
                ...
                else if ((fdstat.fs_rights_base & constants.WASI_RIGHT_FD_WRITE) ===
                    0n) {
        ...
        case "fd_pread":
        case "fd_read": {
            const { sharedBuffer, fd, len, offset } = data;
            const { fds } = processManager.processInfos[processId];
            const lck = new Int32Array(sharedBuffer, 0, 1);
            ...
            if (fds.getDesc(fd) === undefined) {
                ...
                else if (fdstat.fs_filetype === constants.WASI_FILETYPE_SYMBOLIC_LINK) {
                    err = constants.WASI_EINVAL;
                }
                else {
                    let res;
        ...
        case "path_open": {
            const { sharedBuffer, dirFd, path, lookupFlags, openFlags, fsRightsBase, fsRightsInheriting, fdFlags, } = data;
            const lck = new Int32Array(sharedBuffer, 0, 1);
            const openedFd = new Int32Array(sharedBuffer, 4, 1);
            let err;
            const { fds } = processManager.processInfos[processId];
            if (fds.getDesc(dirFd) !== undefined) {
                if (!(openFlags & constants.WASI_O_CREAT &&
                    openFlags & constants.WASI_O_DIRECTORY)) {
                    let desc;
                    ({ err, desc } = await processManager.filesystem.openat(fds.getDesc(dirFd), path, lookupFlags, openFlags, fdFlags, fsRightsBase, fsRightsInheriting, processId));
                    if (err === constants.WASI_ESUCCESS) {
        ...
        case "fd_filestat_get": {
            ...
            if (desc === undefined) {
                ...
                if ((path &&
                    ...
                    let __desc;
                    if (path !== undefined) {
                        let res = await processManager.filesystem.openat(desc, path, lookupFlags, 0, 0, constants.WASI_RIGHTS_ALL, constants.WASI_RIGHTS_ALL, processId);
                        err = res.err;
        ...
        case "path_remove_directory": {
            ...
            const { fds } = processManager.processInfos[processId];
            if (fds.getDesc(fd) !== undefined) {
                err = await processManager.filesystem.removeEntry(path, action !== "path_unlink_file", fds.getDesc(fd), processId);
            }
            else {
                err = constants.WASI_EBADF;
            }
        ...
        case "path_create_directory": {
            ...
            const { fds } = processManager.processInfos[processId];
            if (fds.getDesc(fd) !== undefined) {
                err = await processManager.filesystem.createDir(path, fds.getDesc(fd), processId);
            }
            else {
                err = constants.WASI_EBADF;
            }
        ...
        case "path_rename": {
            const { sharedBuffer, oldFd, oldPath, newFd, newPath } = data;
            const lck = new Int32Array(sharedBuffer, 0, 1);
            const { fds } = processManager.processInfos[processId];
            let err = await processManager.filesystem.move(fds.getDesc(oldFd), oldPath, fds.getDesc(newFd), newPath, processId);
            Atomics.store(lck, 0, err);
            Atomics.notify(lck, 0);
            break;
        }
        case "fd_tell": {
            ...
            if (fds.getDesc(fd) !== undefined) {
                ...
                if ((fdstat.fs_rights_base & constants.WASI_RIGHT_FD_TELL) !== 0n) {
                    if (fdstat.fs_filetype === constants.WASI_FILETYPE_REGULAR_FILE ||
                        fdstat.fs_filetype === constants.WASI_FILETYPE_SYMBOLIC_LINK) {
                        offset[0] = BigInt((await fds.getDesc(fd).seek(0n, constants.WASI_WHENCE_CUR)).offset);
                        err = constants.WASI_ESUCCESS;
                    }
                    else {
                        err = constants.WASI_EBADF;
        ...
        case "fd_filestat_set_times": {
            const { sharedBuffer, st_atim, flags, st_mtim, fst_flags, fd, path } = data;
            ...
            if (desc) {
                let fdstat = await desc.getFdstat();
                if (!(fdstat.fs_rights_base & constants.WASI_RIGHT_FD_FILESTAT_SET_TIMES)) {
                    err = constants.WASI_EACCES;
                }
                else {
                    if (!(((fst_flags & constants.WASI_FSTFLAGS_ATIM_NOW) !== 0 &&
        ...
        case "fd_fdstat_set_flags": {
            const { sharedBuffer, fd, flags } = data;
            const lck = new Int32Array(sharedBuffer, 0, 1);
            const { fds } = processManager.processInfos[processId];
            let err = constants.WASI_ESUCCESS;
            let fdEntry = fds.getFdEntry(fd);
        ...
        case "fd_renumber": {
            const { sharedBuffer, fd, newFd } = data;
            const lck = new Int32Array(sharedBuffer, 0, 1);
            const { fds } = processManager.processInfos[processId];
            let err = constants.WASI_ESUCCESS;
        ...
        case "poll_oneoff": {
            const { sharedBuffer, subs, eventBuf } = data;
            const lock = new Int32Array(sharedBuffer, 0, 2);
            const { fds } = processManager.processInfos[processId];
            let __events = [];
            await Promise.any(subs.map((sub) => {
                let __subPromise;
                switch (sub.eventType) {
                    case constants.WASI_EVENTTYPE_CLOCK:
                        __subPromise = new Promise((resolve) => {
                            setTimeout(() => {
                                resolve({
                            ...
                            }, Number(sub.event.timeout / 1000000n));
                        });
                        break;
                    case constants.WASI_EVENTTYPE_FD_WRITE:
                    case constants.WASI_EVENTTYPE_FD_READ: {
                        const fd = (__subPromise = fds.getDesc(sub.event.fd));
                        if (fd === undefined) {
                            ...
                            __subPromise = fd.addPollSub(sub.userdata, sub.eventType, processId);
                        }
                        break;
                    }
                    default:
                ...
                return __subPromise.then((event) => __events.push(event));
            }));
        ...
        case "event_source_fd": {
            const { sharedBuffer, eventMask } = data;
            const lck = new Int32Array(sharedBuffer, 0, 1);
            const fileDescriptor = new Int32Array(sharedBuffer, 4, 1);
            let eventSource = new EventSource(0, constants.WASI_RIGHTS_ALL, constants.WASI_RIGHTS_ALL, eventMask);
        ...
        case "clean_inodes": {
            ...
            const { sharedBuffer } = data;
            const lck = new Int32Array(sharedBuffer, 0, 1);
            await Promise.all(Object.values(processManager.filesystem.getMounts()).map(async (fs) => {
                if (fs instanceof FsaFilesystem)
                    fs.cleanup();
            }));
            Atomics.store(lck, 0, 0);
            Atomics.notify(lck, 0);
        ...
        case "kill": {
            const { sharedBuffer, processId, signalNumber } = data;
            const lck = new Int32Array(sharedBuffer, 0, 1);
            let exitStatus = constants.WASI_ESUCCESS;
            if (signalNumber !== constants.WASI_SIGKILL) {
                // For now, we support SigKill only
                console.log(`Process=${processId} send usupported singnal: ${signalNumber}!`);
                exitStatus = constants.WASI_EINVAL;
            }
            else if (processManager.processInfos[processId] === undefined) {
                console.log(`Process=${processId} send singnal to process ${processId} that does not exist!`);
        ...
        case "ioctl": {
            const { sharedBuffer, fd, command } = data;
            const lck = new Int32Array(sharedBuffer, 0, 1);
        ...
        case "mount": {
            const { sharedBuffer, sourceFd, sourcePath, targetFd, targetPath, filesystemType, mountFlags, data: data_, } = data;
            let err = constants.WASI_ESUCCESS;
            const lck = new Int32Array(sharedBuffer, 0, 1);
            const { fds } = processManager.processInfos[processId];
            ...
            if (err === constants.WASI_ESUCCESS) {
                err = await processManager.filesystem.addMount(sourceDesc, sourcePath, targetDesc, targetPath, filesystemType, mountFlags, opts, processId);
            }
            Atomics.store(lck, 0, err);
            Atomics.notify(lck, 0);
            break;
