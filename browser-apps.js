...
export async function wget(processManager, processId, args, env, fds) {
    ...
    if (args.length === 2) {
        ...
        if (operator != "-O") {
            await stderr.write(new TextEncoder().encode("wget: help: wget <address> [<path>] or wget <addres> -O <path>\n"));
            return 1;
        }
    }
    else {
        await stderr.write(new TextEncoder().encode("wget: help: wget <address> [<path>]\n"));
        return 1;
    }
    /* if (path == "-") {
      try {
    ...
    try {
        ...
        await stderr.write(new TextEncoder().encode(`wget: could not get resource: ${error.message.toLowerCase()}\n`));
        return 1;
    }
    return 0;
}
...
export async function ps(processManager, _processId, _args, _env, fds) {
    ...
    for (const [id, workerInfo] of Object.entries(processManager.processInfos)) {
        ...
        psData += `${`     ${id}`.slice(-5)} pts/0    ${`00${hours}`.slice(-2)}:${`00${minutes}`.slice(-2)}:${`00${seconds}`.slice(-2)} ${
...
export async function free(processManager, processId, args, _env, fds) {
    const stdout = fds.getDesc(1);
    ...
    freeData += `Mem:      ${`          ${totalMemory}`.slice(-10)}  ${`          ${usedMemory}`.slice(-10)}  ${`          ${availableMemory}`.slice(-10)}\r\n`;
    await stdout.write(new TextEncoder().encode(freeData));
    return constants.EXIT_SUCCESS;
}
export async function reset(_processManager, _processId, _args, _env, _fds) {
    location.reload();
    return constants.EXIT_SUCCESS;
}
